...
function solar_radiation_alloc(latitude, longitude, altitude, utc_time)
    # starts here: https://www.pveducation.org/pvcdrom/properties-of-sunlight/solar-radiation-outside-the-earths-atmosphere
...
function solar_radiation_alloc_typed(
    latitude :: Real,
    altitude :: Real,
    utc_time :: DateTime)
    # starts here: https://www.pveducation.org/pvcdrom/properties-of-sunlight/solar-radiation-outside-the-earths-atmosphere
...
function solar_radiation_alloc_typed_optimized(
    latitude :: Real,
    altitude :: Real,
    utc_time :: DateTime)
    # starts here: https://www.pveducation.org/pvcdrom/properties-of-sunlight/solar-radiation-outside-the-earths-atmosphere
    ...
    # minutes_from_start_of_the_day = Dates.hour.(data_df.utc_time) * 60 .+ Dates.minute.(data_df.utc_time);
    accumulator = 15 * ((Dates.hour(utc_time) * 60 + Dates.minute(utc_time)) /60 - 12)

    sun_declination_angle = -23.45 * cosd(360 / 365 * (Dates.dayofyear(utc_time) + 10))

    elevation = asin(
        sind(sun_declination_angle) * sind(latitude) +
        cosd(sun_declination_angle) * cosd(latitude) * cosd(accumulator)
    )
    # when elevation angle is negative it means that sun is below horizon 
    # so, we should nullify these elements
    # elevation_filtered = copy(elevation)
    ...
    zenith_angle = pi/2. - elevation
    # plot(data_df.utc_time, zenith_angle, title="Zenith angle of the sun in rad")
    accumulator = cos(zenith_angle)

    # solar radiation consists of direct and diffuse radiation
    # https://www.pveducation.org/pvcdrom/properties-of-sunlight/air-mass 

    # calculate air mass - how much air light must pass through
    accumulator = 1 / (accumulator + 0.50572 * (96.07995 - deg2rad(zenith_angle))^-1.6364 )

    # with altitude (from sea level)
    accumulator = sin(elevation) * 1.1 * H_constant * ( (1 - 0.14 * altitude / 1000) * 0.7 ^ (accumulator ^ 0.678) + 0.14 * altitude / 1000) # W/m^2
...
function solar_power_income_alloc(latitude, longitude, altitude, utc_time, diff_distance, speed_vector)
    electrics_efficiency = 0.86
    solar_panels_efficiency = 0.228
    panels_area = 4 # m^2
	return solar_radiation_alloc(
        latitude, longitude, altitude, utc_time
        ) * electrics_efficiency * solar_panels_efficiency * panels_area * diff_distance / speed_vector / 3600.0
end

function solar_power_income_alloc_typed(
    latitude :: Real,
    altitude :: Real,
    utc_time :: DateTime,
    time_s :: Real
    ...
    solar_panels_efficiency = 0.228
    panels_area = 4 # m^2
	return solar_radiation_alloc_typed_optimized(
